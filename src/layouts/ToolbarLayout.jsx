...
function ToolbarLayout() {
  ...
  return (
    <div className="flex fixed h-full text-white border-r pb-10">
      <div
        className={`h-full p-5 pt-8 border-r ${
          open ? "w-64" : isTabletScreen ? "w-16" : "w-20"
        }  ${isTabletScreen ? "fixed" : "relative"} duration-300`}
        style={{ backgroundColor: "#171C2Eff" }}
      >
        <MdOutlineMenuOpen
          className={`hover:text-violet-300 overflow-visible  bg-white text-black text-3xl rounded-md absolute -right-3 top-1/4  border-black border cursor-pointer ${
            !open && "rotate-180"
          }`}
