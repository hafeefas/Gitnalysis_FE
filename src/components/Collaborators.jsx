    ...
    useEffect(() => {
        async function fetchCollaborators() {
            try {
                ...
                const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/collaborators/${username}/${repo}`,{},{withCredentials:true});
                    ...
                    <ul className="rounded-lg">
                        {collaborators.map(collaborator => (
                            <li key={collaborator.login} className="border-b p-2 text-white">
                              <img src={collaborator.avatar_url} alt="avatar" width="30px" height="30px" 
                                style={{display: "inline-block", marginRight: "25px", borderRadius: "100%"}}        
                              />
                              {collaborator.login}
                            </li>
                        ))}
